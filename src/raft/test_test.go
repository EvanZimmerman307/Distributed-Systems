...
func TestFailAgree2B(t *testing.T) {
	...
	fmt.Println("Server: ", (leader+1)%servers, "got disconnected")
...
func TestFailNoAgree2B(t *testing.T) {
	...
	fmt.Println("Server: ", (leader+2)%servers, "got disconnected")
	cfg.disconnect((leader + 3) % servers)
	fmt.Println("Server: ", (leader+3)%servers, "got disconnected")
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	fmt.Println("First Leader: ", leader1, " has disconnected")
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	fmt.Println("Second Leader: ", leader2, " has disconnected")

	// old leader connected again
	cfg.connect(leader1)
	fmt.Println("First Leader reconnected: ", leader1, " has reconnected")

	cfg.one(104, 2, true)

	// all together now
	cfg.connect(leader2)
	fmt.Println("Second Leader reconnected: ", leader2, " has reconnected")
...
func TestPersist12C(t *testing.T) {
	...
	// crash and re-start all, should this cause a new election?
	for i := 0; i < servers; i++ {
		cfg.start1(i, cfg.applier)
		fmt.Println("Server: ", i, "killed and restarted, so it should persist")
	}
	for i := 0; i < servers; i++ {
		cfg.disconnect(i)
		fmt.Println("Server: ", i, "has disconnected")
		cfg.connect(i)
		fmt.Println("Server: ", i, "has reconnected")
	}
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	cfg.start1(leader1, cfg.applier) // Should this lead to a new election?
	fmt.Println("Leader: ", leader1, " killed and disconnected")
	cfg.connect(leader1)
	fmt.Println("Leader: ", leader1, " reconnected")

	cfg.one(13, servers, true)

	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	fmt.Println("Leader: ", leader2, " has disconnected")
	cfg.one(14, servers-1, true)
	cfg.start1(leader2, cfg.applier)
	cfg.connect(leader2)
	fmt.Println("Server: ", leader2, "was killed and then reconnected")
...
func TestPersist22C(t *testing.T) {
	...
	for iters := 0; iters < 5; iters++ {
		...
		cfg.disconnect((leader1 + 1) % servers)
		//fmt.Println("Server: ", (leader1 + 1) % servers, " has disconnected")
		cfg.disconnect((leader1 + 2) % servers)
		//fmt.Println("Server: ", (leader1 + 2) % servers, " has disconnected")

		cfg.one(10+index, servers-2, true)
		index++

		cfg.disconnect((leader1 + 0) % servers)
		//fmt.Println("Server: ", (leader1 + 0) % servers, " has disconnected")
		cfg.disconnect((leader1 + 3) % servers)
		cfg.disconnect((leader1 + 4) % servers)
